...
html
body
	link(rel="stylesheet" href="https://assets.bili33.top/css/APlayer.min.css")	//- 引用修改的CSS
	#aplayer	//- 相当于<div>
	script(src="https://assets.bili33.top/js/APlayer.min.js")		//- 引用js文件
```
...
首先先[下载](https://assets.bili33.top/file/card_aplayer.pug)预设文档，放到主题目录下的`layout/includes/addons`文件夹内（如果不存在请自己创建）
...
![成果图](https://assets.bili33.top/img/butterfly-customize/Netease-Result.png)

我们先打开网易云的一首歌，点击生成外联播放器，要求是这首歌不需要VIP进行下载，否则会二话不说给你弹出下面这个窗

![版 权 保 护](https://assets.bili33.top/img/butterfly-customize/Netease-not-allow.png)

正常情况下，会给你打开外联播放器的生成页面，在下面选择合适的参数，其中宽度我稍微试了一下，推荐为`230`，太小会观感体验不好，太大会直接超出框，设定好宽度我们就复制代码（宽度也可以直接在`width=`后面进行修改哦）

![外链播放器生成页面](https://assets.bili33.top/img/butterfly-customize/Netease-Player-Gen.png)
